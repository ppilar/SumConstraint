...
from .utils import get_nan_entries, omit_nan_covar

#SC changes are marked by comment 'SC:'


#SC: change name of function
def sc_prediction_strategy(train_inputs, train_prior_dist, train_labels, likelihood, mode, fhat, sig, W):
    train_train_covar = train_prior_dist.lazy_covariance_matrix
    if isinstance(train_train_covar, LazyEvaluatedKernelTensor) and not mode == 'L' and not mode == 'var':
        cls = train_train_covar.kernel.prediction_strategy
    else:
        cls = DefaultPredictionStrategy
    return cls(train_inputs, train_prior_dist, train_labels, likelihood, mode, fhat, sig, W)


class DefaultPredictionStrategy(object):
    def __init__(self, train_inputs, train_prior_dist, train_labels, likelihood, mode, fhat, sig, W, root=None, inv_root=None):
        # Get training shape
    ...
    def mean_cache(self):
        ...
        #SC: delete nan entries
        buf, ibuf, N = get_nan_entries(self.train_labels)
        
        #SC: Laplace
        if self.mode == 'L':
            train_labels = self.fhat
            
            #SC: delete nan entries
            ttbuf2 = omit_nan_covar(self.train_labels, self.train_prior_dist.covariance_matrix)                
            train_train_covar2 = lazify(ttbuf2 + 1e-4*torch.eye(N))
        elif self.mode == 'var':
            train_labels = self.fhat
            ...
            train_train_covar2 = lazify(omit_nan_covar(self.train_labels, train_train_covar.evaluate()))
            train_train_covar2 = lazify(train_train_covar2)
            train_labels = self.train_labels[buf]
            

        train_labels_offset = (train_labels - train_mean[buf]).unsqueeze(-1)
        mean_cache = train_train_covar2.evaluate_kernel().inv_matmul(train_labels_offset).squeeze(-1)
    ...
    def exact_prediction(self, joint_mean, joint_covar):
        ...
        #SC: dropout columns of test_train_covar corresponding to missing measurements
        buf = ~torch.isnan(self.train_labels)
        if torch.sum(~buf) != 0:
            if type(test_train_covar) == torch.Tensor:
                test_train_covar2 = test_train_covar[:,buf]                
            else:
                test_train_covar2 = lazify(test_train_covar.evaluate()[:,buf])
        else:
            test_train_covar2 = test_train_covar

        return (
            self.exact_predictive_mean(test_mean, test_train_covar2),
            self.exact_predictive_covar(test_test_covar, test_train_covar2),
        )
    ...
    def exact_predictive_covar(self, test_test_covar, test_train_covar):
        ...
        #SC: dropout r/c in covar_cache:
        buf = ~torch.isnan(self.train_labels)
        ibuf = torch.where(buf)[0]
        N = torch.sum(buf)
        precomputed_cache2 = torch.zeros(N,N)
        for i in range(N):
            precomputed_cache2[i,:] = precomputed_cache[ibuf[i],buf]        
        
        
        covar_inv_quad_form_root = self._exact_predictive_covar_inv_quad_form_root(precomputed_cache2, test_train_covar)
        if torch.is_tensor(test_test_covar):   
            
            #SC
            if self.mode == 'L':
                #omit nan entries - dropout
                train_train_covar = omit_nan_covar(self.train_labels, self.train_prior_dist.covariance_matrix) #K
                try:
                    L = torch.cholesky(train_train_covar + torch.diag(1/torch.diag(self.W)))
                    res = test_test_covar - test_train_covar@torch.linalg.solve(L.T,torch.linalg.solve(L,test_train_covar.T))
                except:
                    off = 1e-4*torch.eye(self.W.shape[0])
                    buf = train_train_covar + torch.diag(1/torch.diag(self.W))
                    res = test_test_covar - test_train_covar@torch.inverse(train_train_covar + buf + off)@test_train_covar.T  #investigate!
                
                return lazify(
                    #test_test_covar - test_train_covar@Kinv@test_train_covar.T + test_train_covar@Kinv@self.sig@Kinv@test_train_covar.T
                    #test_test_covar - test_train_covar@torch.inverse(train_train_covar + buf)@test_train_covar.T
                    res
                )
            elif self.mode == 'var':          
                train_train_covar = omit_nan_covar(self.train_labels, self.train_prior_dist.covariance_matrix)                
                off = 1e-3*torch.eye(train_train_covar.shape[0])
                Kinv = torch.inverse(train_train_covar + off)
                return lazify(
                    #test_test_covar - test_train_covar@Kinv@test_train_covar.T + test_train_covar@Kinv@self.sig@Kinv@test_train_covar.T
                    test_test_covar - test_train_covar@(Kinv - Kinv@self.sig@Kinv.T)@test_train_covar.T
                )
            else:
                return lazify(
                    torch.add(
