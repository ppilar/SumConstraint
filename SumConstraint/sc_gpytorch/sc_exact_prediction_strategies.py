...
def sc_prediction_strategy(train_inputs, train_prior_dist, train_labels, likelihood):
...
class DefaultPredictionStrategy(object):
    ...
    def mean_cache(self):
        ...
        N = torch.sum(buf)
        train_train_covar2 = torch.zeros(N,N)
        tbuf = train_train_covar.evaluate()
        for i in range(N):
            train_train_covar2[i,:] = tbuf[ibuf[i],buf]
        train_train_covar2 = lazify(train_train_covar2)
        
        train_labels_offset = (self.train_labels[buf] - train_mean[buf]).unsqueeze(-1)
        mean_cache = train_train_covar2.inv_matmul(train_labels_offset).squeeze(-1)
