...
class MultitaskMultivariateNormal(MultivariateNormal):
    ...
    def log_prob(self, value):
        ...
        return log_prob_super(value.view(*value.shape[:-2], -1))  #SC: call custom function instead of super()
    
    def log_prob_super(self, value): #SC
        ...
        #SC: remove dropped out data points
        covar = covar.evaluate()
        
        buf = ~torch.isnan(diff)
        ibuf = torch.where(buf)[0]
        N = torch.sum(buf)
        diff2 = diff[ibuf]
        
        covar2 = torch.zeros(N,N)
        for i in range(N):
            covar2[i,:] = covar[ibuf[i],buf]
        
        covar2 = lazify(covar2)
        
        # Get log determininat and first part of quadratic form
        inv_quad, logdet = covar2.inv_quad_logdet(inv_quad_rhs=diff2.unsqueeze(-1), logdet=True)

        res = -0.5 * sum([inv_quad, logdet, diff2.size(-1) * math.log(2 * math.pi)])
        return res
...
#SC changes are marked by comment 'SC:'
class MultitaskMultivariateNormalDropout(MultivariateNormal):
    """
    Constructs a multi-output multivariate Normal random variable, based on mean and covariance
    Can be multi-output multivariate, or a batch of multi-output multivariate Normal
    ...
    def log_prob(self, value):
        ...
        return self.log_prob_super(value.view(*value.shape[:-2], -1))
    
    def log_prob_super(self, value):
        #if settings.fast_computations.log_prob.off():
        #    return super().log_prob(value)
